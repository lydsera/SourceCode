...
$$ b_{ij} \geq 0$$
$$ b_{ij} \leq 0$$  
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;该形式分为$b_{ij}=0$与$b_{ij}=1$便较好理解
<br>
...
### **层级聚类**
&emsp;&emsp;我们的目标是将有关联的被指向对象（比如出现在同一指向集的）分配相近的数字标识符，通过层级聚类来实现这一目标。本文使用自底向上的聚类方式，即从一个object，根据距离最近的原则不断聚合成cluster，直至只剩一个cluster。  
&emsp;&emsp;例如：  
![](img/聚类例子.jpg)  
&emsp;&emsp;在这个例子中，根据坐标的欧式距离，按图(b)序号先后聚合，得到最终一个cluster④，同时得到图(c)这样的层次结构。  
&emsp;&emsp;有三种连接标准来判断是否在cluster之间做聚合操作：  
- 单连接：通过两个cluster内的最近的object间的距离来判断，例如图(b)中聚合3时计算w和y的距离
- 完全连接：同上相反，找最远距离，例如图(b)中聚合3时计算z和y的距离  
- 平均连接：找平均坐标间的距离，例如图(c)中聚合3时计算w和z连线中点到y的距离    

&emsp;&emsp;聚类在本文的数据上运行的非常快，所以文章在三种标准上都运行，最后选出可能得到理想映射的聚类结果。

### **聚类对象**
&emsp;&emsp;想在具体在指向的对象上实现聚类，首先要知道如何定义两个对象之间的距离，就好像上面图三的例子，坐标上的点间的距离可以用欧式距离来衡量。  
&emsp;&emsp;对象间的距离定义为：两个对象之间的距离为两个对象都出现在的指向集的最小word数。  
...
&emsp;&emsp;有了距离矩阵就可以进行聚类，得到树状图。在树状图上进行深度优先搜索得到的对象就是最邻近的对象，配合从零开始的计数器来分配标识符实现映射。

### **基于区域的聚类**
&emsp;&emsp;距离矩阵是对称矩阵，所以可以压缩成上三角矩阵，但矩阵元素为$\frac{n(n-1)}{2}$，内存会随着元素的增加呈二次增长，如下图：  
![](img/fig_4_memory.jpg)  
...
部分区域的对象小于W，该区域只需分配一个word，无需聚类操作来分析它们的邻近关系，直接分配任意标识符即可，文中称这些区域为琐碎区域。  
![](img/table_2.jpg)  
如图，可以看出琐碎区域占了占比很大，大大减少了聚类操作的工作负载。  

## **评估**

### **实现**
本文已在SVF框架下使用fastcluster实现了上述方法，其中稀疏位向量用LLVM中的实现，使用STL向量实现核位向量，标准位向量对应核位向量偏移量为零的情况。  
更多详情看后面代码部分，下面对评估效果略作介绍。  
